...
int
ubpf_load_elf(struct ubpf_vm *vm, const void *elf, size_t elf_size, char **errmsg, uint64_t memory_ptr, uint32_t memory_size)
{
    ...
    int rv = ubpf_load(vm, text_copy, sections[text_shndx].size, errmsg, memory_ptr, memory_size);
    free(text_copy);
    return rv;
