...
//    controller->threshold_times_granularity = 1 - r_times_granularity(controller);  // FIXME: experimental
    ...
    if (controller->md == 0 && controller->ad == 0) {
        // because we have no current feedback information bu no added degree, so we should still protect the window if losses are possible
        controller->d_times_granularity = 1*GRANULARITY;
    } else {
...
//                        if (false && EW(controller, current_window)) {
                    ...
                    case available_slot_reason_ack:;
                        ...
                        if (normal_causal && (EW(controller, current_window) && controller->latest_symbol_when_fec_scheduled != current_window->end-1)
