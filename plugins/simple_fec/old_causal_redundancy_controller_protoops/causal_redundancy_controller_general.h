...
static __attribute__((always_inline)) causal_redundancy_controller_t *create_causal_redundancy_controller(picoquic_cnx_t *cnx, uint32_t m, uint32_t k) {
...
static __attribute__((always_inline)) bool EW(picoquic_cnx_t *cnx, picoquic_path_t *path, causal_redundancy_controller_t *controller, uint64_t granularity, fec_window_t *current_window) {
    protoop_arg_t args[4];
    args[0] = (protoop_arg_t) path;
    args[1] = (protoop_arg_t) controller;
    args[2] = (protoop_arg_t) granularity;
    args[3] = (protoop_arg_t) current_window;
    return run_noparam(cnx, "causal_ew", 4, args, NULL);
}

static __attribute__((always_inline)) int64_t threshold(picoquic_cnx_t *cnx, picoquic_path_t *path, causal_redundancy_controller_t *controller, uint64_t granularity) {
    protoop_arg_t args[3];
    args[0] = (protoop_arg_t) path;
    args[1] = (protoop_arg_t) controller;
    args[2] = (protoop_arg_t) granularity;
    return run_noparam(cnx, "causal_threshold", 3, args, NULL);
}

static __attribute__((always_inline)) bool below_threshold(picoquic_cnx_t *cnx, picoquic_path_t *path, causal_redundancy_controller_t *controller, uint64_t granularity) {
    int64_t th = threshold(cnx, path, controller, granularity);
...
//                        if (allowed_to_send_fec_given_deadlines && ((normal_causal && EW(controller, current_window)) || (!fec_has_protected_data_to_send(cnx) && bw_ratio_times_granularity > (GRANULARITY + GRANULARITY/10)))) {
                    if (EW(cnx, path, controller, GRANULARITY, current_window)) {
                ...
                case available_slot_reason_nack:
                    if (!below_threshold(cnx, path, controller, GRANULARITY)) {
                        if (!EW(cnx, path, controller, GRANULARITY, current_window)) {
                            // TODO: first check if new data are available to send ?
                            added_new_packet = true;
                            add_elem_to_buffer(controller->what_to_send, new_rlnc_packet);
