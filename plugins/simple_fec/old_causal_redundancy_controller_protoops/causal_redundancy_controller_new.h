...
static __attribute__((always_inline)) bool threshold_exceeded(picoquic_cnx_t *cnx, causal_redundancy_controller_t *controller, bandwidth_t available_bandwidth, bandwidth_t used_bandwidth) {
    ...
    PROTOOP_PRINTF(cnx, "%lu - %lu >? %lu: %d\n", r_times_granularity(controller), controller->d_times_granularity, controller->threshold_times_granularity, r_times_granularity(controller) - controller->d_times_granularity > controller->threshold_times_granularity);
...
//    controller->k = 4;//MIN(MAX_SENDING_WINDOW_SIZE/2 - 1, 1 + cwin_max_slots);
    controller->threshold_times_granularity = -(1*GRANULARITY - r_times_granularity(controller));  // FIXME: experimental
    ...
    if (controller->md == 0 && controller->ad == 0) {
        ...
        // no added degrees, but there are missing degrees: we need to retransmit asap
        controller->d_times_granularity = GRANULARITY + 1;
    } else {
...
//    PROTOOP_PRINTF(cnx, "USED BW = %lu, BW_RATIO = %lu, RECEIVE RATE = %lu, DOF_RATIO = %lu/%lu = %lu\n", used_bandwidth_bytes_per_second, GRANULARITY*available_bandwidth_bytes_per_second/used_bandwidth_bytes_per_second, r_times_granularity(controller), controller->md, ad, controller->d_times_granularity);
    ...
    if (is_buffer_empty(controller->what_to_send)) {
        if (controller->flush_dof_mode) {
            ...
            if (false && controller->lost_and_non_fec_retransmitted_slots->current_size > 0) {
...
//                                                                                                                                                     || current_time + next_message_time_to_wait_microsec + owd_microsec + DEADLINE_CRITICAL_THRESHOLD_MICROSEC >= soonest_deadline_microsec); // if false, that means that we can wait a bit before sending FEC
                bool allowed_to_send_fec_given_deadlines = true;
                PROTOOP_PRINTF(cnx, "allowed to send = %d, owd = %lu, current_time = %lu, soonest_deadline = %lu, next_timestamp = %lu, last fb = %d\n", allowed_to_send_fec_given_deadlines,
                               owd_microsec, current_time, soonest_deadline_microsec, wff->next_message_timestamp_microsec, controller->last_feedback);

                bool normal_causal = true;
...
//                        if (allowed_to_send_fec_given_deadlines && ((normal_causal && EW(controller, current_window)) || (!fec_has_protected_data_to_send(cnx) && bw_ratio_times_granularity > (GRANULARITY + GRANULARITY/10)))) {
                        if (allowed_to_send_fec_given_deadlines && ((normal_causal && false && EW(controller, current_window)) || (!fec_has_protected_data_to_send(cnx) && bw_ratio_times_granularity > (GRANULARITY + GRANULARITY/10)))) {
...
//                            if (true || !EW(controller, current_window)) {
                            if (fec_has_protected_data_to_send(cnx)/*!EW(controller, current_window)*/) {
                                // TODO: first check if new data are available to send ?
                                added_new_packet = true;
                                add_elem_to_buffer(controller->what_to_send, new_rlnc_packet);
                            } else {
