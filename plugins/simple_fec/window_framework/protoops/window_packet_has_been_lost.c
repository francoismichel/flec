...
//    uint64_t packet_number = (uint64_t) get_cnx(cnx, AK_CNX_INPUT, 0);
    ...
    window_slot_nacked(cnx, (window_fec_framework_t *) state->framework_sender, packet_slot);
    for (window_source_symbol_id_t id = first_source_symbol_id ; id < first_source_symbol_id + n_source_symbols_in_packet ; id++) {
        sfpid_has_landed(cnx, (window_fec_framework_t *) state->framework_sender, id, false);
    }
    // no slot available
    return 0;
}