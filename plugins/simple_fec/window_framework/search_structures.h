...
static __attribute__((always_inline)) bool is_empty(min_max_pq_t pq) {
    return pq->size == 0;
}
...
static __attribute__((always_inline)) void * get_min(min_max_pq_t pq) {
    if (pq->size == 0)
        return NULL;
    return pq->buffer[ROOT].value;
}

static __attribute__((always_inline)) void * pop_min(min_max_pq_t pq) {
    if (pq->size == 0)
        return NULL;
    void *val = get_min(pq);
...
static __attribute__((always_inline)) void * get_max(min_max_pq_t pq) {
    // the max is the largest child of the root
    return pq->buffer[largest_child_idx(pq, ROOT)].value;
}

static __attribute__((always_inline)) void * pop_max(min_max_pq_t pq) {
    if (pq->size == 0)
        return NULL;
    void *val = get_max(pq);
