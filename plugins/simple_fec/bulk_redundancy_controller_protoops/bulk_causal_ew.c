...
//    PROTOOP_PRINTF(cnx, "BULK EW\n");
    ...
    if(!get_loss_parameters(cnx, path, current_time, granularity, &loss_rate_times_granularity, NULL, &gemodel_r_times_granularity)) {
        max_proportion = window_size(current_window)/3;
    } else {
        max_proportion = window_size(current_window)*loss_rate_times_granularity/GRANULARITY;
    }
    PROTOOP_PRINTF(cnx, "N FEC IN FLIGHT = %lu, LIKELY TO BE LOST = %lu, GEMODEL R = %lu\n", controller->n_fec_in_flight, (1 + MAX(granularity/MAX(1, gemodel_r_times_granularity), max_proportion)), gemodel_r_times_granularity);
