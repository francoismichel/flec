...
protoop_arg_t causal_what_to_send(picoquic_cnx_t *cnx) {
    ...
    PROTOOP_PRINTF(cnx, "IN WHAT TO SEND\n");
    if (!state)
    ...
    if (window_size(&window) > 0) {
        run_algo(cnx, path, (bulk_redundancy_controller_t *) wff->controller, reason, &window);
    }
    PROTOOP_PRINTF(cnx, "DONE RUN ALGO 2\n");
    ...
    PROTOOP_PRINTF(cnx, "BEFORE SET CNX\n");
