#ifndef bulk_redundancy_CONTROLLER_H
#define bulk_redundancy_CONTROLLER_H
#include "picoquic.h"
#include "../fec.h"
...
static __attribute__((always_inline)) void add_elem_to_buffer2(picoquic_cnx_t *cnx, bulk_redundancy_controller_t *controller, buffer_t *buffer, buffer_elem_t slot) {
    uint64_t idx = buffer->start + buffer->current_size;
    idx = idx % (uint64_t) buffer->max_size;
    *(buffer->elems + idx) = slot;
    if (buffer->current_size == buffer->max_size) {
...
static __attribute__((always_inline)) int get_window_sent_at_slot(bulk_redundancy_controller_t *controller, slots_history_t *history, int64_t slot, fec_window_t *w) {
...
static __attribute__((always_inline)) bulk_redundancy_controller_t *create_bulk_redundancy_controller(picoquic_cnx_t *cnx, uint32_t threshold_times_granularity, uint32_t m, uint32_t k) {
    bulk_redundancy_controller_t *controller = my_malloc(cnx, sizeof(bulk_redundancy_controller_t));
    if (!controller) return NULL;
    memset(controller, 0, sizeof(bulk_redundancy_controller_t));
    controller->k = k;
    controller->m = m;
    ...
    PROTOOP_PRINTF(cnx, "controller = %p, controller size = %lu, end = %p, acked slots = %p, acked_slots->elems = %p",
            (protoop_arg_t) controller, sizeof(bulk_redundancy_controller_t), (protoop_arg_t) ((uint8_t *) controller) + sizeof(bulk_redundancy_controller_t),
...
static __attribute__((always_inline)) causal_packet_type_t what_to_send(picoquic_cnx_t *cnx, window_redundancy_controller_t c, window_source_symbol_id_t *first_id_to_protect, uint16_t *n_symbols_to_protect) {
    bulk_redundancy_controller_t *controller = (bulk_redundancy_controller_t *) c;
...
static __attribute__((always_inline)) bool should_send_fec_given_channel(picoquic_cnx_t *cnx, picoquic_path_t *path, bulk_redundancy_controller_t *controller, fec_window_t *current_window, uint64_t current_time) {
    uint64_t loss_times_granularity, p_times_granularity, r_times_granularity;
    get_loss_parameters(cnx, path, current_time, GRANULARITY, &loss_times_granularity, &p_times_granularity, &r_times_granularity);
    PROTOOP_PRINTF(cnx, "N FEC IN FLIGHT = %lu!\n", controller->n_fec_in_flight);
//    return MAX(GRANULARITY/MAX(1, GRANULARITY - loss_times_granularity), GRANULARITY/MAX(1, GRANULARITY-r_times_granularity)) > controller->n_fec_in_flight;
    return MAX(MAX(1, (window_size(current_window)*loss_times_granularity)/GRANULARITY), GRANULARITY/MAX(1U, r_times_granularity)) > controller->n_fec_in_flight;
}

#define SECOND_IN_MICROSEC 1000000

// either acked or recovered
static __attribute__((always_inline)) void run_algo(picoquic_cnx_t *cnx, picoquic_path_t *path, bulk_redundancy_controller_t *controller, available_slot_reason_t feedback, fec_window_t *current_window) {
    ...
    } else if (!fec_has_protected_data_to_send(cnx) && should_send_fec_given_channel(cnx, path, controller, current_window, picoquic_current_time())) {
        PROTOOP_PRINTF(cnx, "ADD FEC TO BUFFER\n");
        add_elem_to_buffer(controller->what_to_send, fec_packet);
        controller->n_fec_in_flight++;
    } else {
        add_elem_to_buffer(controller->what_to_send, new_rlnc_packet);
    }
    PROTOOP_PRINTF(cnx, "DONE RUN ALGO\n");
