#ifndef MESSAGE_BASED_REDUNDANCY_CONTROLLER_H
#define MESSAGE_BASED_REDUNDANCY_CONTROLLER_H
#include "picoquic.h"
#include "../fec.h"
...
} message_based_redundancy_controller_t;
...
static __attribute__((always_inline)) void add_elem_to_buffer2(picoquic_cnx_t *cnx, message_based_redundancy_controller_t *controller, buffer_t *buffer, buffer_elem_t slot) {
    uint64_t idx = buffer->start + buffer->current_size;
    idx = idx % (uint64_t) buffer->max_size;
    *(buffer->elems + idx) = slot;
    if (buffer->current_size == buffer->max_size) {
...
static __attribute__((always_inline)) int get_window_sent_at_slot(message_based_redundancy_controller_t *controller, slots_history_t *history, int64_t slot, fec_window_t *w) {
...
static __attribute__((always_inline)) message_based_redundancy_controller_t *create_message_based_redundancy_controller(picoquic_cnx_t *cnx, int64_t m) {
    message_based_redundancy_controller_t *controller = my_malloc(cnx, sizeof(message_based_redundancy_controller_t));
    if (!controller) return NULL;
    memset(controller, 0, sizeof(message_based_redundancy_controller_t));
    ...
    PROTOOP_PRINTF(cnx, "controller = %p, controller size = %lu, end = %p, acked slots = %p, acked_slots->elems = %p",
            (protoop_arg_t) controller, sizeof(message_based_redundancy_controller_t), (protoop_arg_t) ((uint8_t *) controller) + sizeof(message_based_redundancy_controller_t),
...
static __attribute__((always_inline)) void destroy_message_based_redundancy_controller(picoquic_cnx_t *cnx, message_based_redundancy_controller_t *controller) {
    destroy_buffer(cnx, controller->acked_slots);
    destroy_buffer(cnx, controller->nacked_slots);
    my_free(cnx, controller);
}

static __attribute__((always_inline)) int64_t r_times_granularity(message_based_redundancy_controller_t *controller) {
...
static __attribute__((always_inline)) causal_packet_type_t what_to_send(picoquic_cnx_t *cnx, window_redundancy_controller_t c, window_source_symbol_id_t *first_id_to_protect, uint16_t *n_symbols_to_protect) {
    message_based_redundancy_controller_t *controller = (message_based_redundancy_controller_t *) c;
...
static __attribute__((always_inline)) int64_t get_max_fec_threshold(picoquic_cnx_t *cnx, message_based_redundancy_controller_t *controller, fec_window_t *current_window) {
//    return MAX(window_size(current_window)/3, 1);
    return MAX(3*window_size(current_window)/2, 1);
}

static __attribute__((always_inline)) void compute_new_last_fully_protected_message_deadline(picoquic_cnx_t *cnx, window_fec_framework_t *wff, message_based_redundancy_controller_t *controller, fec_window_t *current_window) {
...
static __attribute__((always_inline)) void run_algo(picoquic_cnx_t *cnx, picoquic_path_t *path, message_based_redundancy_controller_t *controller, available_slot_reason_t feedback, fec_window_t *current_window) {
...
#endif //MESSAGE_BASED_REDUNDANCY_CONTROLLER_H