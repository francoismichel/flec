...
protoop_arg_t parse_frame(picoquic_cnx_t *cnx) {
    ...
    PROTOOP_PRINTF(cnx, "PARSE REPAIR FRAME, SKIP FRAME = %d\n", state->is_in_skip_frame);
    tetrys_repair_frame_t *rf = parse_tetrys_repair_frame(cnx, bytes_protected, bytes_max, &consumed, state->is_in_skip_frame);
