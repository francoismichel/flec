...
protoop_arg_t parse_frame(picoquic_cnx_t *cnx) {
    ...
    tetrys_source_symbol_id_t *id = my_malloc(cnx, sizeof(tetrys_source_symbol_id_t));
    if (!id)
        return PICOQUIC_ERROR_MEMORY;
    *id = 0;
    ...
    size_t consumed = 0;
    int err = decode_tetrys_source_symbol_id(bytes_protected, bytes_max - bytes_protected, id, &consumed);
