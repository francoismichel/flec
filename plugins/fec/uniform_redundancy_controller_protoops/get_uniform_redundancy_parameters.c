...
protoop_arg_t get_constant_redundancy_parameters(picoquic_cnx_t *cnx)
{
    ...
    PROTOOP_PRINTF(cnx, "RETURN UNIFORM PARAMETERS N = %u, K = %u, TOTAL RECEIVED = %lu, TOTAL LOST = %lu, FLUSH = %d\n", n, k, urc->total_acknowledged_packets, urc->total_lost_packets, flush);
    return 0;
}