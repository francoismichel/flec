...
static __attribute__((always_inline)) source_symbol_t *add_source_symbol(picoquic_cnx_t *cnx, received_source_symbols_buffer_t *buffer, source_symbol_t *ss) {
    ...
    if (buffer->first < 0 || (ss->source_fec_payload_id.raw < buffer->source_symbols[buffer->first]->source_fec_payload_id.raw && idx != buffer->first))
        buffer->first = idx;
        ...
        int min_index = -1;
        uint32_t min_val = 0;
        PROTOOP_PRINTF(cnx, "CONSIDER %u SYMBOLS, FIRST = %d, IDX = %d\n", n_considered_symbols, buffer->first, idx);
        for (uint32_t i = 0 ; i < buffer->max_size - 1 && n_considered < buffer->max_size ; i++) {
            ...
            if (buffer->source_symbols[index]) {
                n_considered++;
                if (min_index == -1 || buffer->source_symbols[index]->source_fec_payload_id.raw < min_val) {
                    min_index = index;
                    min_val = buffer->source_symbols[index]->source_fec_payload_id.raw;
                }
                PROTOOP_PRINTF(cnx, "FOUND AT THE %dth ITERATION, AT INDEX %d\n", i, (((uint32_t) buffer->first) + 1U + i) % buffer->max_size);
            }
        }
    ...
    if (buffer->size > 0)
        PROTOOP_PRINTF(cnx, "NEW FIRST = %u\n", buffer->source_symbols[buffer->first]->source_fec_payload_id.raw);
