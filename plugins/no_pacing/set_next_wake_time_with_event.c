...
protoop_arg_t set_next_wake_time(picoquic_cnx_t *cnx)
{
    ...
    uint64_t min_wake_time = UINT64_MAX;
    if (!state) {
        PROTOOP_PRINTF(cnx, "COULD NOT GET NO PACING STATE !\n");
    } else if (state->soonest_event < current_time) {
        state->soonest_event = SOONEST_EVENT_UNDEFINED;
        PROTOOP_PRINTF(cnx, "SOONEST EVENT UNDEFINED\n");
    } else if (state->soonest_event != SOONEST_EVENT_UNDEFINED) {
        min_wake_time = state->soonest_event;
    }
    ...
    PROTOOP_PRINTF(cnx, "CURRENT BLOCKED = %d, PACING = %d\n", blocked, pacing);
    if (blocked == 0 || (wake_now && pacing == 0)) {
            ...
            for (int i = 0; i < nb_paths; i++) {
                ...
                if (ack_needed) {
                    ...
                    if (ack_time < next_time) {
                        next_time = ack_time;
                        PROTOOP_PRINTF(cnx, "UPDATE NEXT TIME = %lu\n", next_time);
                    }
                }
                ...
                if (p != NULL) {
                    ...
                    if (latest_time_acknowledged > send_time    // we already received an acknowledgement for an older packet, so there is a hole. Identical to checking delta_seq > 0
                        ...
                        if (delta_seq < 3) {
                            ...
                            if (next_time < rack_timer_min) {
                                ...
                                PROTOOP_PRINTF(cnx, "UPDATE NEXT TIME FOR RACK = %lu\n", next_time);
                            }
                        }
                    }
                    ...
                    if (nb_retransmit == 0) {
                        ...
                        if (send_time + retransmit_timer_x < next_time) {
                            next_time = send_time + retransmit_timer_x;
                            PROTOOP_PRINTF(cnx, "UPDATE NEXT TIME RTO = %lu\n", next_time);
                        }
                    }
                    else {
                        if (send_time + (1000000ull << (nb_retransmit - 1)) < next_time) {
        ...
        for (int i = 0; i < nb_paths; i++) {
            ...
            if (challenge_verified_x == 0) {
                ...
                if (current_time < next_challenge_time) {
                    if (next_time > next_challenge_time) {
                        next_time = next_challenge_time;
                        PROTOOP_PRINTF(cnx, "UPDATE NEXT TIME CHALLENGE = %lu\n", next_time);
                    }
                }
            }
            ...
            if (keep_alive_interval != 0 && next_time > (latest_progress_time + keep_alive_interval)) {
                ...
                PROTOOP_PRINTF(cnx, "UPDATE NEXT TIME KEEP ALIVE = %lu\n", next_time);
            }
        }
    }
