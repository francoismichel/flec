# 1 "set_next_wake_time_without_pacing.c"
# 1 "<interne>"
# 1 "<ligne-de-commande>"
...
# 1 "set_next_wake_time_without_pacing.c"
# 1 "../../picoquic/picoquic.h" 1
# 25 "../../picoquic/picoquic.h"
...
# 28 "../../picoquic/picoquic.h" 2
# 1 "../../picoquic/protoop.h" 1
# 27 "../../picoquic/protoop.h"
# 1 "../../picoquic/uthash.h" 1
# 29 "../../picoquic/uthash.h"
...
# 32 "../../picoquic/uthash.h" 2
# 1173 "../../picoquic/uthash.h"

# 1173 "../../picoquic/uthash.h"
typedef struct UT_hash_bucket {
   struct UT_hash_handle *hh_head;
   unsigned count;
# 1189 "../../picoquic/uthash.h"
...
{
    uint64_t ret;
    do { unsigned _uthash_hfstr_keylen = (unsigned)strlen(str_pid); do { do { unsigned _fn_i; const unsigned char *_hf_key = (const unsigned char*)(str_pid); (ret) = 2166136261U; for (_fn_i=0; _fn_i < _uthash_hfstr_keylen; _fn_i++) { ret = ret ^ _hf_key[_fn_i]; ret = ret * 16777619U; } } while (0); } while (0); } while (0);
    return ret;
}
# 63 "../../picoquic/protoop.h"
extern protoop_id_t PROTOOP_PARAM_PARSE_FRAME;
# 75 "../../picoquic/protoop.h"
...
# 39 "../../picoquic/picoquic.h" 2
# 117 "../../picoquic/picoquic.h"
...
typedef struct st_picoquic_packet_t {
    ...
    unsigned int is_congestion_controlled : 1;
    unsigned int has_plugin_frames : 1;

    picoquic_packet_plugin_frame_t *plugin_frames;
...
# 301 "../../picoquic/picoquic.h" 3 4
   _Bool 
# 301 "../../picoquic/picoquic.h"
        caller_is_intern;
    int inputc;
    protoop_arg_t *inputv;
    protoop_arg_t *outputv;
} protoop_params_t;
# 315 "../../picoquic/picoquic.h"
...
# 719 "../../picoquic/picoquic.h" 3 4
_Bool 
# 719 "../../picoquic/picoquic.h"
    picoquic_has_booked_plugin_frames(picoquic_cnx_t *cnx);
# 2 "set_next_wake_time_without_pacing.c" 2
# 1 "../../picoquic/plugin.h" 1
# 9 "../../picoquic/plugin.h"
# 1 "../../picoquic/picoquic.h" 1
# 10 "../../picoquic/plugin.h" 2
...
# 11 "../../picoquic/plugin.h" 2
# 1 "/usr/lib/gcc/x86_64-redhat-linux/8/include/stdarg.h" 1 3 4
# 12 "../../picoquic/plugin.h" 2



# 14 "../../picoquic/plugin.h"
typedef enum {
    pluglet_extern,
    pluglet_replace,
    pluglet_pre,
...
# 3 "set_next_wake_time_without_pacing.c" 2
# 1 "../helpers.h" 1
...
static uint32_t helper_get_checksum_length(picoquic_cnx_t* cnx, int is_cleartext_mode)
{
    ...
    return (uint32_t) run_noparam(cnx, "get_checksum_length", 1, args, 
# 71 "../helpers.h" 3 4
                                                                                      ((void *)0)
# 71 "../helpers.h"
                                                                                          );
    ...
    uint64_t lost_packet_number, uint64_t current_time)
{
    ...
    run_noparam(cnx, "congestion_algorithm_notify", 6, args, 
# 108 "../helpers.h" 3 4
                                                                            ((void *)0)
# 108 "../helpers.h"
                                                                                );
    ...
    size_t length, picoquic_call_back_event_t fin_or_event)
{
    ...
    run_noparam(cnx, "callback_function", 4, args, 
# 119 "../helpers.h" 3 4
                                                                  ((void *)0)
# 119 "../helpers.h"
                                                                      );
...
{
    protoop_arg_t args[3], outs[10];
    args[0] = (protoop_arg_t) bytes;
    args[1] = (protoop_arg_t) bytes_max;
    args[2] = (protoop_arg_t) *no_need_to_repeat;
    int ret = (int) run_noparam(cnx, "check_stream_frame_already_acked", 3, args, outs);
...
static uint32_t helper_predict_packet_header_length(picoquic_cnx_t *cnx, picoquic_packet_type_enum packet_type, picoquic_path_t* path_x)
{
    ...
    return (uint32_t) run_noparam(cnx, "predict_packet_header_length", 2, args, 
# 152 "../helpers.h" 3 4
                                                                                               ((void *)0)
# 152 "../helpers.h"
                                                                                                   );
...
static int helper_is_stream_frame_unlimited(const uint8_t* bytes)
{
    ...
    return (((first_byte) & ~((picoquic_frame_type_stream_range_min)^(picoquic_frame_type_stream_range_max)^(0x02))) == (picoquic_frame_type_stream_range_min));
}
...
static void helper_dequeue_retransmit_packet(picoquic_cnx_t* cnx, picoquic_packet_t* p, int should_free)
{
    ...
    run_noparam(cnx, "dequeue_retransmit_packet", 2, args, 
# 167 "../helpers.h" 3 4
                                                                          ((void *)0)
# 167 "../helpers.h"
                                                                              );
...
static int helper_should_send_max_data(picoquic_cnx_t* cnx)
{
    ...
    uint64_t data_received = (uint64_t) get_cnx(cnx, 0x25, 0);
    uint64_t maxdata_local = (uint64_t) get_cnx(cnx, 0x26, 0);
...
static __attribute__((always_inline)) int helper_is_mtu_probe_needed(picoquic_cnx_t* cnx, picoquic_path_t * path_x)
{
    ...
    picoquic_state_enum cnx_state = (picoquic_state_enum) get_cnx(cnx, 0x0e, 0);
    unsigned int mtu_probe_sent = (unsigned int) get_path(path_x, 0x09, 0);
    uint32_t send_mtu_max_tried = (uint32_t) get_path(path_x, 0x16, 0);
    uint32_t send_mtu = (uint32_t) get_path(path_x, 0x15, 0);
...
{
    return (picoquic_stream_head *) run_noparam(cnx, "find_ready_stream", 0, 
# 201 "../helpers.h" 3 4
                                                                                            ((void *)0)
# 201 "../helpers.h"
                                                                                                , 
    ...
    picoquic_path_t* path_x)
{
    ...
    return (int) run_noparam(cnx, "is_ack_needed", 3, args, 
# 211 "../helpers.h" 3 4
                                                                           ((void *)0)
# 211 "../helpers.h"
                                                                               );
...
{
    return (int) run_noparam(cnx, "is_tls_stream_ready", 0, 
# 216 "../helpers.h" 3 4
                                                                           ((void *)0)
# 216 "../helpers.h"
                                                                               , 
    ...
    uint64_t current_time, uint32_t * header_length)
{
    ...
    uint32_t length = (uint32_t) run_noparam(cnx, "prepare_packet_old_context", 6, args, outs);
    ...
    picoquic_packet_t* packet, size_t send_buffer_max, int* is_cleartext_mode, uint32_t* header_length, char **reason)
{
    ...
    int ret = (int) run_noparam(cnx, "retransmit_needed", 7, args, outs);
    ...
    uint8_t* bytes)
{
    ...
    return (uint32_t) run_noparam(cnx, "prepare_mtu_probe", 4, args, 
# 269 "../helpers.h" 3 4
                                                                                    ((void *)0)
# 269 "../helpers.h"
                                                                                        );
    ...
    size_t bytes_max, size_t* consumed, picoquic_path_t * path)
{
    ...
    int ret = (int) run_noparam(cnx, "prepare_path_challenge_frame", 4, args, outs);
    *consumed = (size_t) outs[0];
    return ret;
}
    ...
    uint8_t* bytes, size_t bytes_max, size_t* consumed)
{
    ...
    int ret = (int) run_noparam(cnx, "prepare_ack_frame", 5, args, outs);
    *consumed = (size_t) outs[0];
    return ret;
}
    ...
    uint8_t* bytes, size_t bytes_max, size_t* consumed)
{
    ...
    int ret = (int) run_noparam(cnx, "prepare_crypto_hs_frame", 4, args, outs);
    *consumed = (size_t) outs[0];
    return ret;
}
                                      ...
                                      size_t bytes_max, size_t* consumed)
{
    ...
    args[1] = (protoop_arg_t) bytes_max;
    args[2] = (protoop_arg_t) *consumed;
    int ret = (int) run_noparam(cnx, "prepare_first_misc_frame", 3, args, outs);
    *consumed = (size_t) outs[0];
    return ret;
}
    ...
    uint8_t* bytes, size_t bytes_max, size_t* consumed)
{
    ...
    args[2] = (protoop_arg_t) bytes_max;
    args[3] = (protoop_arg_t) *consumed;
    int ret = (int) run_noparam(cnx, "prepare_max_data_frame", 4, args, outs);
    *consumed = (size_t) outs[0];
    return ret;
}
    ...
    uint8_t* bytes, size_t bytes_max, size_t* consumed)
{
    ...
    args[1] = (protoop_arg_t) bytes_max;
    args[2] = (protoop_arg_t) *consumed;
    int ret = (int) run_noparam(cnx, "prepare_required_max_stream_data_frames", 3, args, outs);
    *consumed = (size_t)outs[0];
    return ret;
}
    ...
    uint8_t* bytes, size_t bytes_max, size_t* consumed)
{
    ...
    args[2] = (protoop_arg_t) bytes_max;
    args[3] = (protoop_arg_t) *consumed;
    int ret = (int) run_noparam(cnx, "prepare_stream_frame", 3, args, outs);
    ...
    picoquic_path_t * path_x, uint64_t current_time)
{
    ...
    *send_length = (size_t) run_noparam(cnx, "finalize_and_protect_packet", 10, args, 
# 386 "../helpers.h" 3 4
                                                                                                     ((void *)0)
# 386 "../helpers.h"
                                                                                                         );
...
static void helper_cnx_set_next_wake_time(picoquic_cnx_t* cnx, uint64_t current_time)
{
    protoop_arg_t args[1];
    args[0] = (protoop_arg_t) current_time;
    run_noparam(cnx, "set_next_wake_time", 1, args, 
# 394 "../helpers.h" 3 4
                                                                   ((void *)0)
# 394 "../helpers.h"
                                                                       );
...
static int helper_connection_error(picoquic_cnx_t* cnx, uint16_t local_error, uint64_t frame_type)
{
    ...
    return (int) run_noparam(cnx, "connection_error", 2, args, 
# 414 "../helpers.h" 3 4
                                                                              ((void *)0)
# 414 "../helpers.h"
                                                                                  );
...
static uint8_t* helper_decode_stream_frame(picoquic_cnx_t* cnx, uint8_t* bytes, const uint8_t* bytes_max, uint64_t current_time)
{
    ...
    args[1] = (protoop_arg_t) bytes_max;
    args[2] = (protoop_arg_t) current_time;
    return (uint8_t *) run_noparam(cnx, "decode_stream_frame", 3, args, 
# 423 "../helpers.h" 3 4
                                                                                       ((void *)0)
# 423 "../helpers.h"
                                                                                           );
    ...
    void **frame, int *ack_needed, int *is_retransmittable)
{
    ...
    args[0] = (protoop_arg_t) bytes;
    args[1] = (protoop_arg_t) bytes_max;
    uint8_t *ret_bytes = (uint8_t *) run_param(cnx, "parse_frame", frame_type, 2, args, outs);
    *frame = (void *) outs[0];
    ...
    picoquic_path_t* path_x)
{
    ...
    return (picoquic_packet_t *) run_noparam(cnx, "update_rtt", 5, args, 
# 519 "../helpers.h" 3 4
                                                                                        ((void *)0)
# 519 "../helpers.h"
                                                                                            );
    ...
    uint64_t current_time)
{
    ...
    args[4] = (protoop_arg_t) current_time;
    int ret = (int) run_noparam(cnx, "process_ack_range", 5, args, outs);
    ...
    picoquic_packet_context_enum pc, picoquic_path_t* path_x)
{
    ...
    args[3] = (protoop_arg_t) pc;
    args[4] = (protoop_arg_t) path_x;
    run_noparam(cnx, "check_spurious_retransmission", 5, args, 
# 547 "../helpers.h" 3 4
                                                                              ((void *)0)
# 547 "../helpers.h"
                                                                                  );
...
static void helper_process_possible_ack_of_ack_frame(picoquic_cnx_t* cnx, picoquic_packet_t* p)
{
    protoop_arg_t args[1];
    args[0] = (protoop_arg_t) p;
    run_noparam(cnx, "process_possible_ack_of_ack_frame", 1, args, 
# 554 "../helpers.h" 3 4
                                                                                  ((void *)0)
# 554 "../helpers.h"
                                                                                      );
    ...
    size_t bytes_max, size_t* consumed)
{
    ...
    args[1] = (protoop_arg_t) bytes_max;
    args[2] = (protoop_arg_t) *consumed;
    int ret = (int) run_noparam(cnx, "process_ack_of_stream_frame", 3, args, outs);
    *consumed = (size_t) outs[0];
    return ret;
}
# 578 "../helpers.h"
    ...
    uint64_t start_range, uint64_t end_range)
{
    ...
    run_noparam(cnx, "process_ack_of_ack_range", 3, args, 
# 653 "../helpers.h" 3 4
                                                                         ((void *)0)
# 653 "../helpers.h"
                                                                             );
}
# 4 "set_next_wake_time_without_pacing.c" 2



static void cnx_set_next_wake_time_init(picoquic_cnx_t* cnx, uint64_t current_time)
{
    uint64_t start_time = (uint64_t) get_cnx(cnx, 0x10, 0);
    uint64_t next_time = start_time + 15000000;
    picoquic_stream_head* stream = 
# 11 "set_next_wake_time_without_pacing.c" 3 4
                                  ((void *)0)
# 11 "set_next_wake_time_without_pacing.c"
                                      ;
    int timer_based = 0;
    int blocked = 1;
    int pacing = 0;
    picoquic_path_t * path_x = (picoquic_path_t *) get_cnx(cnx, 0x2e, 0);
    picoquic_packet_context_t *pkt_ctx;
    int pc_ready_flag = 1 << picoquic_packet_context_initial;
    picoquic_stream_head *tls_stream_0 = (picoquic_stream_head *) get_cnx(cnx, 0x30, 0);

    picoquic_crypto_context_t *crypto_context_1 = (picoquic_crypto_context_t *) get_cnx(cnx, 0x31, 1);
    picoquic_crypto_context_t *crypto_context_2 = (picoquic_crypto_context_t *) get_cnx(cnx, 0x31, 2);
    int nb_paths = (int) get_cnx(cnx, 0x2d, 0);
    void *crypto_context_1_aead_encrypt = (void *) get_crypto_context(crypto_context_1, 0x00);

    picoquic_stream_data *tls_stream_0_send_queue = (picoquic_stream_data *) get_stream_head(tls_stream_0, 0x00);

    if (tls_stream_0_send_queue == 
# 27 "set_next_wake_time_without_pacing.c" 3 4
                                  ((void *)0)
# 27 "set_next_wake_time_without_pacing.c"
                                      ) {
        picoquic_stream_head *tls_stream_1 = (picoquic_stream_head *) get_cnx(cnx, 0x30, 1);
        picoquic_stream_data *tls_stream_1_send_queue = (picoquic_stream_data *) get_stream_head(tls_stream_1, 0x00);
        void *crypto_context_2_aead_encrypt = (void *) get_crypto_context(crypto_context_2, 0x00);
        if (crypto_context_1_aead_encrypt != 
# 31 "set_next_wake_time_without_pacing.c" 3 4
                                            ((void *)0) 
# 31 "set_next_wake_time_without_pacing.c"
                                                 &&
            tls_stream_1_send_queue != 
# 32 "set_next_wake_time_without_pacing.c" 3 4
                                      ((void *)0)
# 32 "set_next_wake_time_without_pacing.c"
                                          ) {
            pc_ready_flag |= 1 << picoquic_packet_context_application;
        }
        else if (crypto_context_2_aead_encrypt != 
# 35 "set_next_wake_time_without_pacing.c" 3 4
                                                 ((void *)0) 
# 35 "set_next_wake_time_without_pacing.c"
                                                      &&
                 tls_stream_1_send_queue == 
# 36 "set_next_wake_time_without_pacing.c" 3 4
                                           ((void *)0)
# 36 "set_next_wake_time_without_pacing.c"
                                               ) {
    ...
    else
    {
        for (picoquic_packet_context_enum pc = 0; blocked == 0 && pc < picoquic_nb_packet_context; pc++) {
            for (int i = 0; blocked == 0 && i < nb_paths; i++) {
                path_x = (picoquic_path_t *) get_cnx(cnx, 0x2e, i);
                pkt_ctx = (picoquic_packet_context_t *) get_path(path_x, 0x21, pc);
                picoquic_packet_t* p = (picoquic_packet_t *) get_pkt_ctx(pkt_ctx, 0x0b);
                ...
                while (p != 
# 58 "set_next_wake_time_without_pacing.c" 3 4
                           ((void *)0)
# 58 "set_next_wake_time_without_pacing.c"
                               )
                {
                    picoquic_packet_type_enum ptype = (picoquic_packet_type_enum) get_pkt(p, 0x08);
                    if (ptype < picoquic_packet_0rtt_protected) {
                        if (helper_retransmit_needed_by_packet(cnx, p, current_time, &timer_based, 
# 62 "set_next_wake_time_without_pacing.c" 3 4
                                                                                                  ((void *)0)
# 62 "set_next_wake_time_without_pacing.c"
                                                                                                      )) {
                            blocked = 0;
                        }
                        break;
                    }
                    p = (picoquic_packet_t *) get_pkt(p, 0x01);
                }
        ...
        if (blocked != 0)
        {
            for (int i = 0; blocked != 0 && pacing == 0 && i < nb_paths; i++) {
                path_x = (picoquic_path_t *) get_cnx(cnx, 0x2e, i);
                uint64_t cwin_x = (uint64_t) get_path(path_x, 0x17, 0);
                uint64_t bytes_in_transit_x = (uint64_t) get_path(path_x, 0x18, 0);
                int challenge_verified_x = (int) get_path(path_x, 0x0a, 0);
                if (cwin_x > bytes_in_transit_x && challenge_verified_x == 1) {
                    if (helper_should_send_max_data(cnx) ||
                        helper_is_tls_stream_ready(cnx) ||
                        (crypto_context_1_aead_encrypt != 
# 89 "set_next_wake_time_without_pacing.c" 3 4
                                                         ((void *)0) 
# 89 "set_next_wake_time_without_pacing.c"
                                                              &&
                        ((stream = helper_find_ready_stream(cnx)) != 
# 90 "set_next_wake_time_without_pacing.c" 3 4
                                                                    ((void *)0) 
# 90 "set_next_wake_time_without_pacing.c"
                                                                         || run_noparam(cnx, "has_congestion_controlled_plugin_frames_to_send", 0, 
# 90 "set_next_wake_time_without_pacing.c" 3 4
                                                                                                                                                                    ((void *)0)
# 90 "set_next_wake_time_without_pacing.c"
                                                                                                                                                                        , 
...
# 90 "set_next_wake_time_without_pacing.c"
        ...
        if (blocked == 0) {
            ...
            next_time = (uint64_t) get_path(path_x, 0x1d, 0);
        }
        else {
            for (picoquic_packet_context_enum pc = 0; pc < picoquic_nb_packet_context; pc++) {
                for (int i = 0; i < nb_paths; i++) {
                    ...
                    int ack_needed = (int) get_pkt_ctx(pkt_ctx, 0x0e);
                    if (ack_needed) {
                        next_time = get_pkt_ctx(pkt_ctx, 0x04) + get_pkt_ctx(pkt_ctx, 0x05);
                    }

                    if (p != 
# 120 "set_next_wake_time_without_pacing.c" 3 4
                            ((void *)0)
# 120 "set_next_wake_time_without_pacing.c"
                                ) {
                        picoquic_packet_type_enum ptype = (picoquic_packet_type_enum) get_pkt(p, 0x08);
                        int pis_evaluated = (int) get_pkt(p, 0x0a);
                        int pcontains_crypto = (int) get_pkt(p, 0x0c);

                        while (p != 
# 125 "set_next_wake_time_without_pacing.c" 3 4
                                   ((void *)0) 
# 125 "set_next_wake_time_without_pacing.c"
                                        &&
                            ...
                            p = (picoquic_packet_t *) get_pkt(p, 0x01);
                            if (p != 
# 130 "set_next_wake_time_without_pacing.c" 3 4
                                    ((void *)0)
# 130 "set_next_wake_time_without_pacing.c"
                                        ) {
                                ptype = (picoquic_packet_type_enum) get_pkt(p, 0x08);
                                pis_evaluated = (int) get_pkt(p, 0x0a);
                                pcontains_crypto = (int) get_pkt(p, 0x0c);
                            }
                        }
                    }

                    if (p != 
# 138 "set_next_wake_time_without_pacing.c" 3 4
                            ((void *)0)
# 138 "set_next_wake_time_without_pacing.c"
                                ) {
                        uint64_t nb_retransmit = (uint64_t) get_pkt_ctx(pkt_ctx, 0x06);
                        uint64_t send_time = (uint64_t) get_pkt(p, 0x04);
                        if (nb_retransmit == 0) {
                            uint64_t retransmit_timer_x = (uint64_t) get_path(path_x, 0x10, 0);
    ...
    for (int i = 0; i < nb_paths; i++) {
        ...
        if (blocked != 0 && challenge_verified_x == 0) {
            uint64_t challenge_time_x = (uint64_t) get_path(path_x, 0x06, 0);
            uint64_t retransmit_timer_x = (uint64_t) get_path(path_x, 0x10, 0);
...
{
    uint64_t current_time = (uint64_t) get_cnx(cnx, 0x00, 0);
    int client_mode = (int) get_cnx(cnx, 0x06, 0);
    uint64_t latest_progress_time = (uint64_t) get_cnx(cnx, 0x17, 0);
    uint64_t next_time = latest_progress_time + 120000000 * (2 - client_mode);
    picoquic_stream_head* stream = 
# 187 "set_next_wake_time_without_pacing.c" 3 4
                                  ((void *)0)
# 187 "set_next_wake_time_without_pacing.c"
                                      ;
    int timer_based = 0;
    int blocked = 1;
    int pacing = 0;
    ...
    int wake_now = get_cnx(cnx, 0x33, 0);
    ...
    for (int i = 0; blocked != 0 && i < nb_paths; i++) {
        picoquic_path_t * path_x = (picoquic_path_t *) get_cnx(cnx, 0x2e, i);
        uint64_t cwin_x = (uint64_t) get_path(path_x, 0x17, 0);
        uint64_t bytes_in_transit_x = (uint64_t) get_path(path_x, 0x18, 0);
    ...
    if (blocked != 0) {
        for (int i = 0; blocked != 0 && pacing == 0 && i < nb_paths; i++) {
            ...
            for (picoquic_packet_context_enum pc = 0; pc < picoquic_nb_packet_context; pc++) {
                ...
                if (p != 
# 230 "set_next_wake_time_without_pacing.c" 3 4
                        ((void *)0) 
# 230 "set_next_wake_time_without_pacing.c"
                             && ret == 0 && helper_retransmit_needed_by_packet(cnx, p, current_time, &timer_based, 
# 230 "set_next_wake_time_without_pacing.c" 3 4
                                                                                                                   ((void *)0)
# 230 "set_next_wake_time_without_pacing.c"
                                                                                                                       )) {
                ...
                if (cwin_x > bytes_in_transit_x) {
                    if (helper_should_send_max_data(cnx) ||
                        ...
                        ((cnx_state == picoquic_state_client_ready || cnx_state == picoquic_state_server_ready) &&
                         (stream = helper_find_ready_stream(cnx)) != 
# 245 "set_next_wake_time_without_pacing.c" 3 4
                                                                    ((void *)0)
# 245 "set_next_wake_time_without_pacing.c"
                                                                        )) {
                        blocked = 0;
                    }
                }
    ...
    if (blocked == 0 || (wake_now && pacing == 0)) {
        ...
        for (picoquic_packet_context_enum pc = 0; pc < picoquic_nb_packet_context; pc++) {
            for (int i = 0; i < nb_paths; i++) {
                ...
                int ack_needed = (int) get_pkt_ctx(pkt_ctx, 0x0e);
                if (ack_needed) {
                    uint64_t ack_time = get_pkt_ctx(pkt_ctx, 0x04) + get_pkt_ctx(pkt_ctx, 0x05);
                ...
                if (p != 
# 274 "set_next_wake_time_without_pacing.c" 3 4
                        ((void *)0)
# 274 "set_next_wake_time_without_pacing.c"
                            ) {
                    uint64_t latest_time_acknowledged = (uint64_t) get_pkt_ctx(pkt_ctx, 0x09);
                    uint64_t send_time = (uint64_t) get_pkt(p, 0x04);
                    picoquic_packet_type_enum ptype = (picoquic_packet_type_enum) get_pkt(p, 0x08);
                    if (latest_time_acknowledged > send_time
                        && send_time + 10000 < next_time
                        && ptype != picoquic_packet_0rtt_protected) {
                        next_time = send_time + 10000;
                    }
        ...
        for (int i = 0; i < nb_paths; i++) {
            ...
            uint64_t keep_alive_interval = (uint64_t) get_cnx(cnx, 0x2c, 0);
