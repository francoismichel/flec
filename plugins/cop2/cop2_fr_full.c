...
# 7 "/usr/include/bits/types/struct_tm.h" 3 4
struct tm
{
  int tm_sec;
  int tm_min;
...
# 1 "../../picoquic/endianness.h" 1



# 3 "../../picoquic/endianness.h"
uint16_t my_htons(uint16_t x);
uint16_t my_ntohs(uint16_t x);
# 7 "bpf.h" 2

# 1 "../../picoquic/memory.h" 1
...
static __attribute__((always_inline)) cop2_conn_metrics *initialize_metrics_data(picoquic_cnx_t *cnx)
{
...
{
    int allocated = 0;
    cop2_conn_metrics **bpfd_ptr = (cop2_conn_metrics **) get_opaque_data(cnx, 0x02, sizeof(cop2_conn_metrics *), &allocated);
    if (!bpfd_ptr) return 
# 81 "bpf.h" 3 4
                         ((void *)0)
# 81 "bpf.h"
...
# 84 "bpf.h" 3 4
                     1
# 84 "bpf.h"
                                    , &((*bpfd_ptr)->handshake_metrics.t_start));
    }
    return *bpfd_ptr;
}
...
static __attribute__((always_inline)) int copy_path(char *dst, cop2_path_metrics *path) {
    ...
    long elapsed = (((path->t_end.tv_sec - path->t_start.tv_sec) * 1000) + (((unsigned long)(path->t_end.tv_nsec - path->t_start.tv_nsec)) / ((unsigned int) 1000000)));
...
{
    cop2_path_metrics *prev_path = 
# 132 "bpf.h" 3 4
                                  ((void *)0)
# 132 "bpf.h"
                                      ;
    cop2_path_metrics *path_metrics = metrics->established_metrics;
    struct sockaddr_storage *path_local_addr = (struct sockaddr_storage *) get_path(path, 0x02, 0);
    struct sockaddr_storage *path_peer_addr = (struct sockaddr_storage *) get_path(path, 0x00, 0);
    while(path_metrics != 
# 136 "bpf.h" 3 4
                         ((void *)0) 
# 136 "bpf.h"
                              && (picoquic_compare_addr((struct sockaddr *)&path_metrics->local_addr, (struct sockaddr *)path_local_addr)) && (picoquic_compare_addr((struct sockaddr *)&path_metrics->peer_addr, (struct sockaddr *)path_peer_addr))) {
...
# 142 "bpf.h"
        ...
        picoquic_connection_id_t *initial_cnxid = (picoquic_connection_id_t *) get_cnx(cnx, 0x0f, 0);
        picoquic_connection_id_t *remote_cnxid = (picoquic_connection_id_t *) get_path(path, 0x1f, 0);
        picoquic_connection_id_t *local_cnxid = (picoquic_connection_id_t *) get_path(path, 0x1e, 0);
...
static __attribute__((always_inline)) void complete_path(cop2_path_metrics *path_metrics, picoquic_cnx_t *cnx, picoquic_path_t *path) {
    ...
    picoquic_connection_id_t *remote_cnxid = (picoquic_connection_id_t *) get_path(path, 0x1f, 0);
    picoquic_connection_id_t *local_cnxid = (picoquic_connection_id_t *) get_path(path, 0x1e, 0);
    uint8_t initial_cnxid_len = (uint8_t) get_cnxid(initial_cnxid, 0x01);
    uint8_t remote_cnxid_len = (uint8_t) get_cnxid(remote_cnxid, 0x01);
    uint8_t local_cnxid_len = (uint8_t) get_cnxid(local_cnxid, 0x01);
...
{
    picoquic_packet_t *p = (picoquic_packet_t *) get_cnx(cnx, 0x00, 0);
    helper_protoop_printf(cnx, "FRT fired %p\n", (protoop_arg_t[]){(protoop_arg_t) p}, (sizeof((protoop_arg_t[]){(protoop_arg_t) p})/sizeof(protoop_arg_t)));
    picoquic_path_t *path = (picoquic_path_t *) get_pkt(p, 0x02);
